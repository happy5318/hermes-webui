...
**Fix.** Update to a build containing the prefer_cache wait-gate fix (see PR #7568): cache-only callers now bypass the rebuild wait and serve the warm/disk/minimal catalog — nothing to configure. The same contract makes `prefer_cache` and `force_refresh` mutually exclusive; the contradictory combination is rejected at entry. If your network blackholes the EC2 metadata address, setting `AWS_EC2_METADATA_DISABLED=true` in the WebUI service environment additionally removes the ~4.3s probe from the rebuild itself.
