...
**Fix.** Update to a build containing the prefer_cache wait-gate fix (see PR #7568): the session-open display path resolves via `get_available_models(prefer_cache=True, wait_for_inflight_rebuild=False)` — it never starts a rebuild, never waits for one that is in flight, and never queues on the rebuild lock; it serves the warm in-memory / disk cache or the network-free minimal catalog immediately. The same non-blocking contract applies to the session-visit display fallback and the server-initiated wakeup path. (Routing-authoritative catalog reads — the foreign-session provider repair on the chat/send path — keep `wait_for_inflight_rebuild=True` and still join an in-flight rebuild so a stale snapshot can never route a turn to the wrong backend.) On the pure display side nothing needs configuring. The `prefer_cache` + `force_refresh` combination is contradictory and rejected at entry. If your network blackholes the EC2 metadata address, setting `AWS_EC2_METADATA_DISABLED=true` in the WebUI service environment additionally removes the ~4.3s probe from the rebuild itself.
