...
| Model context / `context_messages` | Supplies conversation state to the agent | Must include the current visible user turn unless deliberately excluded with a user-visible reason; context-only repairs return independent rows for any sanitized entry | Let the agent resume from context that contradicts what the user can see, or mutate the visible transcript while repairing provider-facing history |
